...
const getters = {
  ...
  platformById: (state) => (id) => {
    return [...state.platforms, ...state.customPlatforms].find(
      (p) => p.id === id
    );
  },

  platformsByCategory: (state) => (category) => {
    return [...state.platforms, ...state.customPlatforms].filter(
  ...
  async saveCustomPlatforms({ state }) {
    try {
      localStorage.setItem(
        "custom-platforms",
        JSON.stringify(state.customPlatforms)
      );
    } catch (error) {
