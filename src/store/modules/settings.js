...
async saveSettings({ state, dispatch, commit }) {
    try {
      ...
      // 2. 针对不同改动，调用对应副作用
      if (diffKeys.includes('language') || diffKeys.some(k => k.startsWith('language.'))) {
        
        await dispatch('applyLanguage', state.settings.language)
      }
      if (diffKeys.includes('theme') || diffKeys.some(k => k.startsWith('theme.'))) {
      ...
      if (window.electronAPI?.saveSettings) {
        const success = await window.electronAPI.saveSettings(JSON.parse(JSON.stringify(state.settings)))
