  ...
  (function initChromeAPI() {
    ...
    window.chrome.notifications = {
      ...
      create: function (notificationId, options, callback) {
        console.log(
          "[NotificationIntercept] ✅ Intercepted LINE Chrome notification:",
        ...
        const containerId =
          urlParams.get("containerId") || `container_line_${Date.now()}`;
        ...
        const notificationData = {
          ...
          webNotificationId: `line_chrome_${Date.now()}_${Math.random()
            .toString(36)
            .substr(2, 9)}`,
          timestamp: Date.now(),
        ...
        try {
          ipcRenderer
            .invoke("send-intercepted-notification", notificationData)
            .then((result) => {
              console.log(
                "[NotificationIntercept] ✅ LINE notification sent to main process:",
                result
              );
            })
            .catch((error) => {
              console.error(
                "[NotificationIntercept] ❌ Failed to send LINE notification:",
                error
              );
            });
        } catch (error) {
          console.error(
            "[NotificationIntercept] ❌ LINE IPC communication error:",
            error
          );
        }
...
function injectNotificationIntercept(platformId) {
  console.log(
    `[NotificationIntercept] Preparing intercept for platform: ${platformId}`
  );
...
const chatContextExtractors = {
  whatsapp: () => {
    ...
    try {
      ...
      const contactName = currentChatHeader?.querySelector(
        '[data-testid="contact-name"]'
      )?.textContent;
      const contactPhone = currentChatHeader
        ?.querySelector('[title*="+"]')
        ?.getAttribute("title");
  ...
  telegram: () => {
    ...
    try {
      ...
      const chatSubtitle =
        document.querySelector(".chat-subtitle")?.textContent;
...
function performNotificationIntercept(platformId) {
  try {
    ...
    console.log(
      `[NotificationIntercept] 🚀 Performing intercept for: ${platformId}`
    );

    const OriginalNotification = window.Notification;

    window.Notification = function (title, options = {}) {
      console.log("[NotificationIntercept] ✅ Intercepted notification:", {
        title,
        options,
      });

      // 获取基本信息
      const webNotificationId =
        "web_notif_" +
        Date.now() +
        "_" +
      ...
      let finalContainerId =
        containerIdFromUrl || `container_${finalPlatformId}_${Date.now()}`;
      ...
      try {
        ...
        console.warn(
          "[NotificationIntercept] Failed to extract sender info:",
          error
        );
      }
      ...
      try {
        ipcRenderer
          ...
          .then((result) => {
            console.log(
              "[NotificationIntercept] ✅ Sent to main process:",
              result
            );
          })
          .catch((error) => {
            console.error(
              "[NotificationIntercept] ❌ Failed to send to main process:",
              error
            );
          });
      } catch (error) {
        console.error(
          "[NotificationIntercept] ❌ IPC communication error:",
          error
        );
      }
    ...
    window.Notification.requestPermission = function () {
      console.log(
        "[NotificationIntercept] Permission requested, returning granted"
      );
      return Promise.resolve("granted");
    };

    console.log(
      `[NotificationIntercept] ✅ Enhanced notification API intercepted for: ${platformId}`
    );
  } catch (error) {
    console.error(
      "[NotificationIntercept] ❌ Failed to inject notification intercept:",
      error
    );
  }
}
