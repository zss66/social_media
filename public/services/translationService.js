...
class TranslationService {
   ...
   */
  async translate(text, targetLang, sourceLang = 'auto', provider = 'google', apiKey = '') {
    try {
      if (!text || !text.trim()) {
      ...
      if (apiKey) {
        ...
        const response = await fetch(url, {
          ...
          body: JSON.stringify({
            ...
            source: sourceLang === 'auto' ? undefined : this.getLanguageCode(sourceLang, 'google')
          })
        })
        ...
        console.warn('未提供API密钥，使用免费接口可能不稳定')
         const response = await fetch('http://119.45.9.228:83/v1/completion-messages', {
  method: 'POST',
  headers: {
  ...
  body: JSON.stringify({
    ...
    response_mode: 'blocking',  // 或改为非流式模式
    user: 'test-zss'
  })
})
  ...
  async simulateTranslation(text, targetLang, sourceLang) {
    ...
    if (translations[targetLang] && translations[targetLang][text]) {
      return {
        ...
        sourceLanguage: sourceLang === 'auto' ? this.simpleLanguageDetection(text) : sourceLang
      }
    }
